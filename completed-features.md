...
| **Automatic CHANGELOG.md** | Auto-generated by [git-cliff](https://git-cliff.org/) on every `v*.*.*` tag; follows Keep a Changelog format with conventional-commit grouping; committed back to `main` via the `changelog` workflow |
