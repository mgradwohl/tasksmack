...
| Item | Notes |
|------|-------|
| **PGO (Profile-Guided Optimization) workflow** | `pgo-generate`/`pgo-use` CMake presets (Linux + Windows); `tools/pgo.sh` / `tools/pgo.ps1` automate the full instrumented-build → run → merge → optimised-build pipeline using Clang's `llvm-profdata` |
