...
| Item | Notes |
|------|-------|
| **PGO (Profile-Guided Optimization) workflow** | `pgo-generate`/`pgo-use` CMake presets (Linux + Windows); `tools/pgo.sh` / `tools/pgo.ps1` automate the full instrumented-build → run → merge → optimized-build pipeline using Clang's `llvm-profdata` |
| **Google Benchmark suite** | `benchmarks/` – History, ProcessModel, SystemModel, Format, NetlinkSocketStats benchmarks; memory-growth tracking via `/proc/self/status` (Linux only — Windows benchmarks run without this metric); `benchmark` and `win-benchmark` CMake presets |
