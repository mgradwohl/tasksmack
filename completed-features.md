...
| **Google Benchmark suite** | `benchmarks/` – History, ProcessModel, SystemModel, Format benchmarks (Linux + Windows); NetlinkSocketStats benchmarks (Linux only); memory-growth tracking via `/proc/self/status` (Linux only — Windows benchmarks run without this metric); `benchmark` and `win-benchmark` CMake presets |
