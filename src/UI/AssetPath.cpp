                                      ...
                                      const std::function<bool(const std::filesystem::path&)>& probeExists)
{
    ...
    const std::filesystem::path APP_NAME{TASKSMACK_PROJECT_NAME_LOWER};
    ...
    const std::array<std::filesystem::path, 3> candidates = {
        exeDir / "assets",
        exeDir / DATADIR / APP_NAME / "assets",
        (exeDir / ".." / DATADIR / APP_NAME / "assets").lexically_normal(),
    };
...
std::filesystem::path findAssetsDir()
{
    ...
    static const auto result = selectAssetsDir(executableDir(),
                                               ...
                                               {
                                                   std::error_code ec;
                                                   return std::filesystem::exists(p / "fonts", ec) && !ec;
                                               });
    return result;
}
