...
- **ShellLayer:** main tabs, global settings (refresh cadence, theme, column visibility), shared selection state. A non-owning `PanelTabs` registry binds each owned panel to its event name and cached-label provider in one constructor list. The first registration is the default tab; registration order drives tab display, attach, update, and event forwarding, with reverse-order detach. `Panel::renderContent()` renders the selected tab without a window wrapper. Cross-panel model sharing and selected-process snapshot updates remain explicit shell coordination.
